    ...
    local function FarmPetGui()
        ...
        local function hasTargetAilment(targetKind)
            ...
            for _, ailment in pairs(ailments) do
                for x,y in pairs(ailment) do
                    if x == targetKind then
                        print("true")
                        return true
                    end
                end
            ...
            while true do
                while getgenv().PetFarmGuiStarter do
                    ...
                    repeat task.wait(5)
                        ...
                        -- Check if 'moon' is in the PetAilmentsArray
                        if table.find(PetAilmentsArray, "moon") or table.find(BabyAilmentsArray, "moon") then
                            --print("going moon")
                            taskName = "🌔"
                            getgenv().fsys = require(game:GetService("ReplicatedStorage").ClientModules.Core.ClientData)
                            game:GetService("ReplicatedStorage").API:FindFirstChild("LocationAPI/SetLocation"):FireServer("moon")
                            teleportPlayerNeeds(0, 350, 0)
                            createPlatform()
                            equipPet()
                            repeat task.wait(1)
                            until not hasTargetAilment("moon") and not ClientData.get_data()[game.Players.LocalPlayer.Name].ailments_manager.baby_ailments["moon"]
                            task.wait(2)
