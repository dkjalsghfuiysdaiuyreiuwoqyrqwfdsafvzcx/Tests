...
highestLegendaryLevel = highestLegendaryLevel or 0
highestLegendaryLevelSecond = highestLegendaryLevelSecond or 0
highestOtherLevel = highestOtherLevel or 0
highestOtherLevelSecond = highestOtherLevelSecond or 0

local function equipPet()
    getgenv().fsys = require(game:GetService("ReplicatedStorage").ClientModules.Core.ClientData)
    
    if fsys.get("equip_manager").pets[1] 
       and fsys.get("equip_manager").pets[1].unique 
       and petToEquip ~= fsys.get("equip_manager").pets[1].unique then
        ...
        for _, v in pairs(fsys.get("inventory").pets) do
            ...
            if v.kind ~= "practice_dog" then  -- Ignore practice_dog
                if rarity:lower() == "legendary" then
                    -- Legendary branch: prioritize best and second-best legendary pets
                    if not petToEquip then
                        highestLegendaryLevel = petLevel
                        petToEquip = v.unique
                        namePetToEquip = v.kind
                    elseif petToEquip ~= v.unique then
                        if not petToEquipSecond then
                    ...
                    -- Non-legendary branch: fill petToEquip if not set, then petToEquipSecond if different
                    if not petToEquip then
                        highestOtherLevel = petLevel
                        petToEquip = v.unique
                        namePetToEquip = v.kind
