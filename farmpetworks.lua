...
if not _G.ScriptRunning then
    ...
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    local ContentPacks = ReplicatedStorage.SharedModules:WaitForChild("ContentPacks")

    local petRarityCache = nil      -- [petName] = "Rarity"
    local notFoundCache = {}        -- memoize misses
    ...
    local function buildCache()
        ...
        for _, pack in ipairs(ContentPacks:GetChildren()) do
            if pack:IsA("Folder") then
                ...
                if inv and inv:FindFirstChild("Pets") then
                    local ok, pets = pcall(require, inv.Pets)
                    if ok and type(pets) == "table" then
                        for _, petTbl in pairs(pets) do
                            if type(petTbl) == "table" then
                                local petName
                                local rarity = petTbl.rarity

                                -- find the pet name (string value)
                                for _, v in pairs(petTbl) do
                                ...
                                if petName and rarity then
                                    -- Capitalize first letter using ONLY string.upper
                                    local fixedRarity = string.upper(rarity:sub(1,1)) .. rarity:sub(2)

                                    petRarityCache[petName] = fixedRarity
                                    petRarityCache[string.lower(petName)] = fixedRarity
                                end
                            end
                        end
                    end
        ...
        notFoundCache = {} -- reset misses
    end

    -- auto-rebuild when new packs appear
    ContentPacks.ChildAdded:Connect(function()
        petRarityCache = nil
    end)

    local function CheckRarity(petName)
        if type(petName) ~= "string" then return nil end
        if not petRarityCache then buildCache() end

        -- fast exact name
        local r = petRarityCache[petName]
        if r then return r end

        -- fast lowercase name
        local key = string.lower(petName)
        r = petRarityCache[key]
        if r then return r end

        -- memoize misses
        if notFoundCache[key] then return nil end
    ...
    local function equipPet()
        ...
        game:GetService("ReplicatedStorage"):WaitForChild("adoptme_new_net"):WaitForChild("adoptme_new.modules.Dailies.DailiesNetService:15"):FireServer(unpack(args))
            ...
            if stateTable then
                ...
                if stateTable.pet_rarity ~= nil then
                    ...
                    if dailyName == "higher_maintenance" then
                        requiredNeon = true
                    elseif dailyName == "new_parent" then
                        requiredRarity = "Egg"
                    else
                        requiredNeon = false
                    end
