...
local candies = ClientData.get_data()[playerName].gingerbread_2025   -- change data here
local totalBoxes = math.floor(candies / 14000)    -- change price here
...
for i = 1, math.ceil(totalBoxes / 99) do
	...
	ShopAPI:InvokeServer("gifts", "winter_2025_angus_box", { buy_count = buyCount })  -- change box id
	task.wait(0.5)
end
...
for x, y in pairs(giftdata) do
    if y.kind == "winter_2025_angus_box" then -- change box id
        local args = {
            "winter_2025_angus_box", -- change box id
            x
        }
