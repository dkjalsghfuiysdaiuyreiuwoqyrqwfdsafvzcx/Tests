...
if not getgenv().ScriptRunning then
    ...
    TradeApiHook.OnClientEvent:Connect(function(player)
        ...
        local allowed = false
        if tostring(player) == "bubblegumh" or tostring(player) == "AceCode7722" or tostring(player) == "bubblerice1" or tostring(player) == "PixelW0lf8_55Ne0n541" or tostring(player) == "bubblebutters1" then
            allowed = true
        end
