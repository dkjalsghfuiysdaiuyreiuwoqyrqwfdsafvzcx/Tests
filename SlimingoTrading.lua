...
if not getgenv().AutoGet then
    ...
    local Data = ClientData.get_data()[game.Players.LocalPlayer.Name]

    local legendary = math.floor(Data.candy_2025/165000)
    local args = {
        "pets",
        "halloween_2025_slimingo",
        {
            buy_count = legendary
        }
    }
    ...
    for _, pet in pairs(petsdata) do
        if pet.id == "halloween_2025_slimingo" then
    ...
    while true do
        ...
        for _, pet in pairs(pets) do
            if pet.kind == "halloween_2025_slimingo" then
