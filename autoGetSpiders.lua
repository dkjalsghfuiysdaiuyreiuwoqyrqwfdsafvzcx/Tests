...
local totalBoxes = math.floor(candies / 18000)
...
for i = 1, math.ceil(totalBoxes / 99) do
	...
	ShopAPI:InvokeServer("gifts", "halloween_2025_spider_box", { buy_count = buyCount })
	task.wait(0.5)
end
...
for x, y in pairs(giftdata) do
    if y.kind == "halloween_2025_spider_box" then
        local args = {
...
if not getgenv().AutoGet then
    ...
    for _, pet in pairs(petsdata) do
        if pet.id == "halloween_2025_spider_4" or pet.id == "halloween_2025_spider_5" then
    ...
    while true do
        ...
        for _, pet in pairs(pets) do
            if pet.kind == "halloween_2025_spider_4" or pet.id == "halloween_2025_spider_5" then
