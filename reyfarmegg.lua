        ...
        if getgenv().PrioritizeLegs then
            ...
            if not success or not fsys then
                ...
                if equipManagerPets and equipManagerPets[1] and equipManagerPets[1].kind then
                    ...
                    if petToEquip == nil or (currentPetUnique ~= petToEquip) or (eggToFarmExist and getgenv().eggToFarm ~= currentPetKind) or (not currentPetKind:lower():match("egg$") and Cash > 750 and getgenv().AutoBuyEggs and CheckRarity(currentPetKind) ~= "legendary") or (CheckRarity(currentPetKind) == "legendary" and currentPetAge == 6) then
