...
-- ============================================================
-- FORCE-POLL SIGNALS
-- depositReadySignal: fired when user->bot1 deposit finishes
--                     → wakes bot1->bot2 polling loop early
-- withdrawReadySignal: fired when bot2->bot1 withdraw finishes
...
-- Waits up to maxSeconds, but returns early if signal fires.
-- ============================================================
...
local function handleFindUsernamePetTypeId(username, pets)
    ...
    if not userId then warn("userId missing in response:", raw1) return false end
    ...
    if type(validPets) ~= "table" or #validPets == 0 then warn("No pets found/created:", raw2) return false end
...
local function buildOfferItems(offer)
    ...
    for _, item in pairs(offer.items or {}) do
        ...
        if category == "pets" then
            ...
            print("Skipping non-pet item: " .. tostring(item.kind) .. " (" .. category .. ")")
        end
    end
    return out
end
...
local function notifyBackendDone(username, note)
    print("Notify Backend Done: " .. tostring(note))
end
...
local function handleWithdraw(username)
    ...
    for _, datapet in pairs(data.pets) do
        ...
        if petUnique then
            ...
            warn("Pet not in bot inventory: " .. tostring(datapet.pet_type.petkind))
        end
    end
...
local function confirmWithdrawByTrade(tradeId, username, withdrawItems)
    ...
    local pickedIds = {}
    for _, it in ipairs(withdrawItems or {}) do
        local k = buildKey(it.petkind, it.variant, it.fly, it.ride)
        local arr = idsByKey[k]
        if arr and #arr > 0 then
            table.insert(pickedIds, table.remove(arr, 1))
        end
    end
...
game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("DataAPI/DataChanged").OnClientEvent:Connect(function(...)
            ...
            if depositItems and #depositItems > 0 then
                ...
                if s2 == 200 and d2 and d2.success and d2.existing_after then
                    ...
                    for _, inPet in ipairs(depositItems) do
                        ...
                        if idByKey[k] then table.insert(resolvedPetTypeIds, idByKey[k]) end
                    end
                end
                ...
                if #resolvedPetTypeIds > 0 then
                    ...
                    -- 🔥 Wake up bot1->bot2 deposit polling loop immediately
                    task.wait(2)
                    depositReadySignal:Fire()
                else
                    print("⚠️ No valid pet type IDs — skipping createBotProgress")
                end
            else
                print("ℹ️ User gave no pets — skipping deposit")
            end
    ...
    if recipientName == getgenv().BOT2_NAME then
        ...
        if snapshot.senderConfirmed and snapshot.recipientConfirmed and not finalizedTrades[tradeId] then
            ...
            print("✅ Bot1->Bot2 deposit complete:", tradeId)
        end
    end

    -- -------------------------------------------------------
    -- WITHDRAW: BOT2 -> BOT1 (bot1 is recipient)
    -- -------------------------------------------------------
    if senderName == getgenv().BOT2_NAME then
        ...
        if snapshot.senderConfirmed and snapshot.recipientConfirmed and not finalizedTrades[tradeId] then
            ...
            getgenv().IN_TRADE      = false
            getgenv().IN_TRADE_BOT2 = false
            -- 🔥 Wake up bot1->user withdraw polling loop immediately
            task.wait(2)
    ...
    if getgenv().TRADE_TYPE == "WITHDRAW" and senderName ~= getgenv().BOT2_NAME then
        ...
        if recipient.negotiated and not sender.negotiated then
            print("🔄 User negotiated — bot re-accepting...")
            game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("TradeAPI/AcceptNegotiation"):FireServer()
        end
        ...
        if sender.confirmed and recipient.confirmed and not finalizedTrades[tradeId] then
            ...
            if withdrawOk then
                ...
                if wStatus == 200 and wData and wData.data and wData.data.id then
                    ...
                    print("✅ Withdraw progress DONE:", wData.data.id)
                else
                    warn("⚠️ Could not fetch withdraw record:", wRaw)
                end
                ...
                warn("❌ Backend withdraw failed!")
                game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("TradeAPI/DeclineTrade"):FireServer()
                markTradeDone(tradeId, false)
                notifyBackendDone(username, "Withdraw API failed")
                getgenv().IN_TRADE   = false
                getgenv().TRADE_TYPE = nil
            end
        end
...
-- Wakes immediately when depositReadySignal fires (user->bot1 done)
    ...
    while true do
        ...
        if getgenv().IN_TRADE == false then
            ...
            if data and #data > 0 then
                ...
                if not pData then
                    print("All deposit records in-flight, skipping")
                    continue
                end
...
-- Wakes immediately when withdrawReadySignal fires (bot2->bot1 done)
-- Falls back to 60s poll
-- ============================================================
task.spawn(function()
    while true do
        ...
        if getgenv().IN_TRADE == false then
            ...
            if data and #data > 0 then
                ...
                -- Bot1 already has the pets from the bot2->bot1 trade.
                -- Just need to wait for the user to send a trade request.
                -- The TradeRequestReceived handler will call handleWithdraw(username).
                -- We simply mark that we are expecting a withdraw for this user.
                print("✅ Withdraw record ready for user:", pData.username)
                -- Nothing more needed here — user initiates the trade to bot1
                -- and TradeRequestReceived + handleWithdraw handles the rest.
            end
        end
    end
end)
