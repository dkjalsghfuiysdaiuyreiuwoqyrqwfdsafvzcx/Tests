if not _G.ScriptRunning then
    ...
    local function FarmPetGui()
        ...
        local function CheckRarity(petname)
                        ...
                        for _, Pet in pairs(require(z.InventorySubDB.Pets)) do 
                            for _, b in pairs(Pet) do
                                if tostring(b) == petname then
                                    return Pet.rarity or "Unknown" -- Return immediately if found
                                end
                            end
                        end
                    end
            ...
            return "Not Found" -- Return a default value if pet isn't found
        end
        ...
        for _, mapName in ipairs(maps) do
            for i = 1, 999 do
                local args = {
                    [1] = mapName,
                    [2] = tostring(i),
                    [3] = true -- Second argument is the current number from 1 to 100
                }
            ...
            while true do
                while getgenv().PetFarmGuiStarter do
                    ...
                    repeat task.wait(5)
                        ...
                        if table.find(PetAilmentsArray, "pet_me") and not getgenv().SkipPetMe then
                            --print("going pet me")
                            taskName = "👋"
                            equipPet()
                            task.wait(3)
            ...
            if ClientData.get_data()[game.Players.LocalPlayer.Name].inventory.toys then 
                for i, v in pairs(ClientData.get_data()[game.Players.LocalPlayer.Name].inventory.toys) do
                    ...
                    if v.id == "trade_license" then
                        ...
                        game:GetService("ReplicatedStorage").API:FindFirstChild("SettingsAPI/SetBooleanFlag"):FireServer("tutorial_v3_completed",true)
                        local ClientData = require(game:GetService("ReplicatedStorage").ClientModules.Core.ClientData)
                        local RouterClient = require(game:GetService("ReplicatedStorage").ClientModules.Core.RouterClient.RouterClient)
                        ...
                        task.wait(1)
                        for i, v in pairs(RouterClient.get("trade_license_quiz_manager")["quiz"]) do
